              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(child: _InfoCard('Selling Price',
                            '${AppConstants.currencySymbol} ${NumberFormat('#,##,##0.00').format(product.sellingPrice)}',
                            AppTheme.successColor, Icons.sell_outlined)),
                        const SizedBox(width: 12),
                        Expanded(child: _InfoCard('Cost Price',
                            '${AppConstants.currencySymbol} ${NumberFormat('#,##,##0.00').format(product.costPrice)}',
                            AppTheme.infoColor, Icons.money_off_outlined)),
                      ],
                    ),
                    const SizedBox(height: 12),
                    Row(
                      children: [
                        Expanded(child: _InfoCard('Stock',
                            '${product.stockQuantity} ${product.unit ?? ''}',
                            product.isLowStock ? AppTheme.warningColor : AppTheme.primaryColor,
                            Icons.layers_outlined)),
                        const SizedBox(width: 12),
                        Expanded(child: _InfoCard('Profit Margin',
                            '${product.profitMargin.toStringAsFixed(1)}%',
                            AppTheme.accentColor, Icons.trending_up_rounded)),
                      ],
                    ),
                    const SizedBox(height: 20),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            _DetailRow('Min Stock Alert', '${product.minStockAlert}'),
                            ...
                            _DetailRow('Added', DateFormat(AppConstants.dateFormat).format(product.createdAt)),
                          ],
                        ),
                      ),
                    ),
...
class _Placeholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Center(
        child: Icon(Icons.inventory_2_outlined, size: 80, color: AppTheme.primaryColor),
      ),
    );
  }
}
