...
class StaffMember {
  ...
  final bool isActive;
  const StaffMember({required this.id, required this.userId, required this.role,
      required this.email, required this.fullName, required this.isActive});
...
final staffProvider = AsyncNotifierProvider<StaffNotifier, List<StaffMember>>(() {
        ...
        data: (staff) {
          if (staff.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  const Icon(Icons.group_outlined, size: 56, color: AppTheme.lightTextHint),
                  const SizedBox(height: 16),
                  Text('No staff members yet', style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(height: 8),
                  Text('Invite staff to help manage your business.', style: Theme.of(context).textTheme.bodySmall),
                  const SizedBox(height: 16),
                  ElevatedButton.icon(
                    onPressed: () => _showInviteDialog(context),
                    icon: const Icon(Icons.person_add_outlined),
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final roleColor = _roleColors[member.role] ?? AppTheme.primaryColor;
              ...
              return Card(
                child: ListTile(
                  ...
                  title: Text(member.fullName, style: Theme.of(context).textTheme.titleMedium),
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Invitation sent! (placeholder)'),
